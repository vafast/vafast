...
import { createAuth, createRoleAuth, createPermissionAuth } from "../src/middleware/auth";
import { 
  generateToken, 
  createTokenPair, 
  refreshToken,
...
const writePermissionAuth = createPermissionAuth(["write"], { secret: SECRET_KEY });
...
if (import.meta.main) {
  const port = 3000;
  console.log(`🚀 认证示例服务器启动在 http://localhost:${port}`);
  console.log("\n📋 可用路由:");
